...
- [ ] [CORE] Multi-turn context: `agent/loop.py` accepts an optional `history: list[Turn]` (a bounded window — last 3-5 question/answer/citation turns) so follow-ups like "why does that happen" resolve against the prior turn instead of being treated as an isolated question. No session store, no user identification: a session is scoped to one browser tab/window, held in memory only for that connection's lifetime, and discarded on close/reload — this is a UI-layer concern (M5's `gr.State`), not a backend one. `agent/loop.py` just needs to accept and use the history it's handed; it doesn't own where that history lives.
  ✔ Done when: a two-turn test ("what does nn.Dropout do?" then "why does that help with overfitting?") shows the second answer's retrieval/query-terms step account for the first turn's topic.
...
- [ ] [CORE] Minimal Gradio interface: question field, answer with highlighted quotes, clickable citations (each showing its `license` string beneath it), and a "quotes verified ✓" indicator. Conversation history (3.1) is held in a `gr.State` scoped to that browser tab's connection — per-window, in-memory, no user identification, gone on close/reload. No session store, no Redis, no DB table for this.
